...
public class LocalizationContainer {
    ...
    public LocalizationContainer(JavaPlugin plugin) {
        this.plugin = plugin;
        log.severe("INITIALIZING LOCALLOADER");
        try {
            this.load();
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public void load() throws IOException, NullPointerException {
      ...
      this.resourceBundle = ResourceBundle.getBundle(LOCALIZATION_FILE_NAMES, Locale.getDefault(), loader);
    }
    ...
    private void copyDefaultLocalization() throws IOException, NullPointerException {
        ...
        var listedFiles = configFolder.listFiles((dir, name) -> name.startsWith(LOCALIZATION_FILE_NAMES) && name.endsWith(".properties"));
        if(listedFiles == null || listedFiles.length == 0) {
            var defaultLocalizationFile = Thread.currentThread().getContextClassLoader().getResourceAsStream(LOCALIZATION_FILE_NAMES + ".properties");
            if(defaultLocalizationFile == null) {
                throw new NullPointerException("Default localization.properties file is not present. Please contact the developer");
            }
            Files.copy(defaultLocalizationFile, Paths.get(configFolder.toURI().resolve(LOCALIZATION_FILE_NAMES + ".properties")), StandardCopyOption.REPLACE_EXISTING);
        }
    }
}
