...
public class BanCommand implements CommandExecutor {
    ...
    @Inject
    public BanCommand(Context context, LocalizationContainer localizationContainer, BanFacade banFacade) {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!(sender instanceof ConsoleCommandSender) && !sender.hasPermission("playlegend.ban")) {
            context.runInContext(sender, (ctxSender) -> {
                ctxSender.sendMessage(LocalHelper.translate(this.resourceBundle, LocalKeys.NO_PERMISSION));
                return null;
            });
            return true;
        }
        ...
        if (args.length < 1) {
            context.runInContext(sender, (ctxSender) -> {
                ctxSender.sendMessage(LocalHelper.translate(this.resourceBundle, LocalKeys.NOT_ENOUGH_ARGS));
                return null;
            });
            return false;
        }
        ...
        if (player == null) {
            context.runInContext(sender, ContextProperties.of(LocalePlaceholderHelper.buildTargetPlayerContext(args[0])), (ctxSender, ctxProps) -> {
                ctxSender.sendMessage(LocalHelper.translate(this.resourceBundle, context, LocalKeys.PLAYER_NOT_ONLINE));
                return null;
            });
            //ChatColor.RED + "The mentioned player is not online: " + args[0]);
            return true;
        }
        ...
        try {
            ...
            context.runInContext(sender, ContextProperties.of(LocalePlaceholderHelper.buildAllowedUnitsContext()), (ctxSender, ctxProps) -> {
                ctxSender.sendMessage(LocalHelper.translate(this.resourceBundle, context, LocalKeys.INVALID_DURATION_UNIT));
                return null;
            });
        ...
        try {
            ...
            context.runInContext(sender, ContextProperties.of(LocalePlaceholderHelper.buildBanContext(ban)), (ctxSender, ctxProps) -> {
                player.kickPlayer(LocalHelper.translate(this.resourceBundle, context, LocalKeys.BAN_MESSAGE));
                return null;
            });

            return true;
        } catch (SQLException e) {
            sender.sendMessage(LocalHelper.translate(this.resourceBundle, context, LocalKeys.PLAYER_BANNED_ERROR));//"There was an error whilst banning the player " + player.getName());
